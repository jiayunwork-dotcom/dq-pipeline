...
var (
	// ErrSchemaCorrupt indicates checksum mismatch when loading schema.
	ErrSchemaCorrupt = errors.New("schema: checksum mismatch")

	// ErrSchemaNoChecksum indicates missing checksum in schema file.
	ErrSchemaNoChecksum = errors.New("schema: missing checksum")
)

// SaveSchema writes a schema to a JSON file with SHA-256 checksum trailer.
...
// LoadSchema reads a schema from a checksummed file and verifies integrity.
...
// SchemaEqual returns true if two schemas have the same columns in the same order
