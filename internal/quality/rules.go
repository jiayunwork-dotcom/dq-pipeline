...
// EvaluateExtended evaluates extended rule types beyond the basic set.
// Supported kinds: enum, length, date, type, crossfield.
...
func evalType(t *parse.Table, ci, ri int, rule Rule) []Violation {
	expectedType := rule.Pattern // "int", "float", "bool"
	var viols []Violation
	for rowi, row := range t.Rows {
		val := cellAt(row, ci)
		if val == "" {
...
func evalCrossfield(t *parse.Table, idx map[string]int, ri int, rule Rule) ([]Violation, error) {
	// Pattern format: "col1 op col2" where op is gt/lt/eq/gte/lte
...
// Summarize returns a text summary of violations grouped by column.
...
// MergeViolations combines two violation slices, deduplicating by (Rule, Column, Row).
