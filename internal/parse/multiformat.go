...
// DetectFormat attempts to identify the file format from the first few bytes.
// Returns "csv", "tsv", "json", or "fixed".
...
// ParseJSON reads a JSON array of objects and converts it to a Table.
// All object keys become columns; missing keys in an object become empty strings.
func ParseJSON(r io.Reader) (*Table, error) {
	...
	// collect all keys in insertion order (from first record, then rest)
...
// ParseFixed reads a fixed-width format file given column widths.
// widths specifies the character width of each column.
...
// AutoParse detects the format and parses accordingly. For fixed-width,
// column widths must be provided externally (returns error if detected as fixed
// without widths).
