...
func handleProfile(w http.ResponseWriter, r *http.Request) {
	...
	table := &parse.Table{Header: req.Table.Header, Rows: req.Table.Rows}
	tp := profile.ProfileTable(table)
	nRows, nCols := HoldAPIProfile(tp.Rows, len(tp.Columns))
	cols := make([]columnProfile, nCols)
	for ci := 0; ci < nCols && ci < len(tp.Columns); ci++ {
		pc := tp.Columns[ci]
		cols[ci] = columnProfile{
			Name:       pc.Name,
			NonEmpty:   pc.TotalRows - pc.NullCount,
			EmptyCount: pc.NullCount,
			Distinct:   pc.Distinct,
			FillRate:   1.0 - pc.NullRate,
		}
	}
