// Package transform provides column-level data transformations for a parsed Table.
//
// Transformations include:
//   - Rename columns
//   - Type coercion (to_int, to_float, to_lower, to_upper, to_date)
//   - Derived columns (concat, substring, replace)
//   - Row filtering (keep/drop by predicate)
//   - Deduplication by column key
//   - Column reordering and selection
//
// All transforms are non-destructive: they return a new Table, leaving the
// original unchanged.
package transform
...
// Op defines a single transformation operation.
type Op struct {
	Kind    string // rename, to_lower, to_upper, to_int, to_float, concat, substr, replace, filter, dedup, select
	Column  string // target column
	NewName string // for rename
	Columns []string // for concat, select
	Sep     string // for concat separator
	Start   int    // for substr
	End     int    // for substr (exclusive)
	Old     string // for replace
	New     string // for replace
	// filter
	Predicate string // "notempty", "gt:N", "lt:N", "eq:V", "contains:V"
}

// Apply runs a sequence of operations on the table, returning a new table.
